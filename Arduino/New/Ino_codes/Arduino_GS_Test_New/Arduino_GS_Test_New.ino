...
// if y barley moves inc if it oscillates dec
const float Kp = 2000.0f;   
...
if (controlMode == AUTO && gps.valid) {
  ...
  // 1. Feedback (WHERE WE ARE)
  currentAz = (float)tic1.getCurrentPosition() * 360.0f / stepsPerRevolution;
  currentEl = (float)tic2.getCurrentPosition() * 360.0f / stepsPerRevolution;

  // 2. Commanded angles (WHERE WE WANT TO BE)
  x_value = constrain(x_value, -180.0f, 180.0f);
  y_value = constrain(y_value, -5.0f, 90.0f);

  // 3. Error
  float azError = x_value - currentAz;

  if (azError > 180) azError -= 360;
  if (azError < -180) azError += 360;

  float elError = y_value - currentEl;

  // 4. Control law
  speedX = constrain(azError * Kp, -maxSpeed, maxSpeed);
  speedY = constrain(elError * Kp, -maxSpeed, maxSpeed);
